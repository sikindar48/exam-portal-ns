...
import { questionsApi, questionFoldersApi } from "@/integrations/turso/client";
...
export default function QuestionsManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    if (editingQuestion) {
      const { error } = await questionsApi.update(editingQuestion.id, { ...formData });
      if (error) {
      ...
      const { error } = await questionsApi.create({
        ...formData,
        client_id: clientId,
  ...
  const handleCreateFolder = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { error } = await questionFoldersApi.create({
  ...
  const handleMoveQuestion = async () => {
    ...
    const { error } = await questionsApi.update(moveQuestionTarget.id, { folder_id: folderId });
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    } else {
  ...
  const handleDeleteFolder = async (id: string) => {
    const { error } = await questionFoldersApi.delete(id);
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    } else {
  ...
  const handleDelete = async (id: string) => {
    setLoading(true);
    const { error } = await questionsApi.delete(id);
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    } else {
  ...
  const handleBulkDelete = async () => {
    setLoading(true);
    const { error } = await questionsApi.bulkDelete(selectedIds);
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    } else {
  ...
  const handleBulkMove = async () => {
    ...
    const folderId = selectedMoveFolder === "none" ? null : selectedMoveFolder;
    setLoading(true);
    const { error } = await questionsApi.bulkMove(selectedIds, folderId);
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    } else {
