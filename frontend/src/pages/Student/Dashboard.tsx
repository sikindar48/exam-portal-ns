...
export default function StudentDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (attemptsResult.error) console.error("Attempts fetch error:", attemptsResult.error);

      if (clientId) {
        const clientResult = await clientsApi.get(clientId);
        if (clientResult.data) {
          setClientInfo(clientResult.data);
        }
      }

      const allAttempts = attemptsResult.data || [];

      // Count per test and grab 5 most recent for display
