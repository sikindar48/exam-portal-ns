...
export function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sections = ["features", "pricing", "why-choose", "faq"];
      let current = "";
      ...
      for (const section of sections) {
        ...
        if (element) {
          ...
          // Adjust 100 as needed for top offset (navbar height)
    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-200 dark:border-slate-900 bg-white/70 dark:bg-slate-950/70 backdrop-blur-xl transition-colors">
      <div className="container mx-auto px-4 md:px-8 py-3.5 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
          <a href="#features" className={`transition-colors ${activeSection === "features" ? "text-blue-600 dark:text-blue-400" : "hover:text-slate-900 dark:hover:text-white"}`}>Features</a>
          <a href="#pricing" className={`transition-colors ${activeSection === "pricing" ? "text-blue-600 dark:text-blue-400" : "hover:text-slate-900 dark:hover:text-white"}`}>Pricing</a>
          <a href="#why-choose" className={`transition-colors ${activeSection === "why-choose" ? "text-blue-600 dark:text-blue-400" : "hover:text-slate-900 dark:hover:text-white"}`}>Why Choose Us</a>
          <a href="#faq" className={`transition-colors ${activeSection === "faq" ? "text-blue-600 dark:text-blue-400" : "hover:text-slate-900 dark:hover:text-white"}`}>FAQ</a>
        </nav>
