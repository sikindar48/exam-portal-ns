    ...
    <footer className="shrink-0 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 md:px-6 h-16 flex items-center justify-between gap-2 md:gap-4">
      ...
      <div className="flex items-center gap-1.5 md:gap-2">
        <button
          ...
          title={isMarked ? "Unmark Review" : "Mark for Review"}
          className={`flex items-center justify-center gap-1 md:gap-1.5 h-9 w-9 md:w-auto px-0 md:px-4 text-xs font-semibold border transition-colors rounded-none ${
            isMarked
        ...
        <button
          onClick={onClear}
          title="Clear Answer"
          className="flex items-center justify-center gap-1 md:gap-1.5 h-9 w-9 md:w-auto px-0 md:px-4 text-xs font-semibold rounded-none border border-red-200 dark:border-red-900/50 bg-red-50/50 dark:bg-red-950/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 hover:border-red-300 transition-colors"
        >
      ...
      {/* Middle: Submit Test for Mobile */}
      {onSubmit && (
        <button
          onClick={onSubmit}
          disabled={disableSubmit}
          className="flex md:hidden items-center justify-center h-9 px-3.5 text-xs font-bold uppercase tracking-widest bg-green-600 hover:bg-green-700 text-white disabled:opacity-40 disabled:pointer-events-none transition-colors border border-green-700"
        >
          Submit
        </button>
      )}
      ...
      <div className="flex items-center gap-1.5 md:gap-2">
        ...
        <button
          ...
          className="flex items-center h-9 px-2 md:px-5 text-xs font-semibold rounded-none bg-blue-600 text-white hover:bg-blue-700 transition-colors disabled:opacity-40 disabled:pointer-events-none"
        >
          <span>Next</span>
