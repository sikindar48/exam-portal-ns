...
void mainImage(out vec4 o, vec2 C) {
  ...
  o = vec4(colr, uOpacity * clamp(max(max(colr.r, colr.g), colr.b) * 2.0, 0.0, 1.0));
}
...
const Lightfall = ({
  ...
  const geometryRef = useRef<ReturnType<typeof Triangle> | null>(null);
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      (uniforms.iResolution.value as number[]).splice(0, 3, gl.drawingBufferWidth, gl.drawingBufferHeight, 1);
    };
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (mouseDampening <= 0) {
        (uniforms.iMouse.value as number[]).splice(0, 2, x, y);
      }
    };
    if (mouseInteraction) {
    ...
    const loop = (t: number) => {
      ...
      (uniforms.iTime.value as unknown as number);
