...
import { parseCSV, downloadCSVTemplate, ParsedQuestion } from '@/utils/csvParser';
...
export default function CSVImport({ clientId, onImportComplete }: CSVImportProps) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (parsedQuestions.length > 0) {
        ...
        const validationErrs = validateQuestions(parsedQuestions);
  ...
  const handleImport = async () => {
    ...
    const questionsToImport = questions.map(({ rowNumber, ...q }) => ({
      ...q,
      client_id: clientId,
    }));
    ...
    for (let i = 0; i < questionsToImport.length; i += batchSize) {
      ...
      setImportProgress(Math.round(((i + batch.length) / questionsToImport.length) * 100));
    }
    ...
    if (successCount > 0) {
      toast({
        ...
        description: `${successCount} questions imported successfully${failedCount > 0 ? `, ${failedCount} failed` : ''}`,
      });
      onImportComplete();
    }

    if (failedCount === questionsToImport.length) {
      toast({
        title: 'Import Failed',
        description: 'All questions failed to import. Please check the format and try again.',
        variant: 'destructive',
      });
    }
  };
  ...
  const validQuestions = questions.filter(q => 
    !validationErrors.some(e => e.row === q.rowNumber)
  );
        ...
        <div className="space-y-4">
          ...
          {questions.length > 0 && validationErrors.length === 0 && (
            <Alert>
              ...
              <AlertDescription>
                {validQuestions.length} valid questions ready to import
              </AlertDescription>
            </Alert>
          )}
                ...
                <Table>
                  ...
                  <TableBody>
                    {questions.slice(0, 50).map((q) => {
                      const hasError = validationErrors.some(e => e.row === q.rowNumber);
                      return (
                        <TableRow key={q.rowNumber} className={hasError ? 'bg-destructive/10' : ''}>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              disabled={questions.length === 0 || validationErrors.length > 0 || importing || importResult !== null}
            >
