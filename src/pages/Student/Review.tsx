...
export default function Review() {
  ...
  const loadReviewData = async () => {
    ...
    try {
      ...
      setTestInfo(attemptData.tests);

      // Verify if review is permitted
      if (!attemptData.tests?.allow_review) {
      ...
      // 4. Fetch test questions and inner question records
      ...
      const groupedSections: Section[] = (sectionsData || []).map((sec) => ({
        id: sec.id,
        name: sec.name,
        position: sec.position,
        questions: validTQs.filter((tq) => tq.section_id === sec.id),
      }));

      // Find any independent questions not assigned to a section
      const independentTQs = validTQs.filter((tq) => !tq.section_id);
      if (independentTQs.length > 0) {
        groupedSections.push({
          id: "general",
          name: "General Section",
          position: -1,
          questions: independentTQs,
        });
      }

      setSections(groupedSections.filter(s => s.questions.length > 0));
    } catch (err: any) {
