...
import { LogOut, Users, Building, FileQuestion, ClipboardList, TrendingUp } from 'lucide-react';
import { supabase } from '@/integrations/supabase/client';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, PieChart, Pie, Cell } from 'recharts';

const COLORS = ['hsl(210, 95%, 45%)', 'hsl(195, 70%, 50%)', 'hsl(142, 76%, 36%)', 'hsl(38, 92%, 50%)', 'hsl(0, 84%, 60%)'];
...
export default function SuperAdminDashboard() {
  ...
  const fetchStats = async () => {
    const [clients, profiles, questions, tests, attempts] = await Promise.all([
      ...
      supabase.from('attempts').select('id, test_id, score, total_marks, status'),
    ]);
    ...
    if (clients.data && profiles.data) {
      ...
      setClientData(Array.from(countMap.entries()).map(([id, count]) => ({
    ...
    if (clients.data && tests.data) {
      ...
      tests.data.forEach(t => {
        if (t.client_id) {
          testCountMap.set(t.client_id, (testCountMap.get(t.client_id) || 0) + 1);
        }
      });
      setAttemptsByClient(Array.from(testCountMap.entries()).map(([id, count]) => ({
        name: clientMap.get(id) || 'Unknown',
        value: count,
      })));
    }
    ...
    <div className="min-h-screen bg-muted">
      ...
      <main className="container mx-auto p-6 space-y-6">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          {[
            { label: 'Total Clients', value: stats.totalClients, icon: Building },
            { label: 'Total Students', value: stats.totalStudents, icon: Users },
            { label: 'Total Questions', value: stats.totalQuestions, icon: FileQuestion },
            { label: 'Total Tests', value: stats.totalTests, icon: ClipboardList },
            { label: 'Total Attempts', value: stats.totalAttempts, icon: TrendingUp },
              ...
              {clientData.length > 0 ? (
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={clientData}>
                    ...
                    <YAxis />
                    <Tooltip />
                    <Bar dataKey="students" fill="hsl(210, 95%, 45%)" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
          ...
          <Card>
            ...
            <CardContent>
              {attemptsByClient.length > 0 ? (
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie data={attemptsByClient} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label>
